...
export class graph {
	constructor(context, settings) {
		...
		var merge = helpers.merge = function (target, source) {
			...
			var functionsAry = [], tempParts = tempStr.split("<br>");

			for (var i = 0; i < tempParts.length; i++) {
				var functionComp = "return '" + tempParts[i]
					.split("<").join("'+ dataset.")
					.split(">").join("+'") + "'";
		...
		if (!this.dynamicZoom)
			console.warn("Dynamic zoom was turned off due to the fact that the scale is stretched!");
		...
		this.drawGrid = function () {
			...
			var canvasWidth = this.canvasWidth = this.canvas.width, canvasHeight = this.canvasHeight = this.canvas.height, range = {
		...
		this.getIncrement = function (units, increment) {
			if (typeof increment === "undefined")
				var increment = 1;
			if (units / increment > 35)
				increment = this.getIncrement(units, increment + 1);
		...
		this.funGraph = function (funcStr, color) {
			var xx, yy, xN, yN, previous = { x: 0, y: 0 }, dx = 4, points = [], middleX = this.middleX, middleY = this.middleY, iMax = Math.round((this.canvasWidth - middleX) / dx), iMin = Math.round(-middleX / dx), func = math.compile(funcStr);

			this.ctx.beginPath();
			this.ctx.lineWidth = 2;
			this.ctx.strokeStyle = color;
			for (var i = iMin; i <= iMax; i++) {
				xx = dx * i; yy = this.unitsGapY * func.evaluate({ x: xx / this.unitsGapX });
				if (middleY - yy >= 0) {
					...
					xN = xx + dx, yN = this.unitsGapY * func.evaluate({ x: xN / this.unitsGapX });
		...
		this.addGraphs = function (graphFuncs) {
			...
			var graphReturn, prevLenght = this.functions.length;
			...
			for (var i = 0; i < this.functions.length; i++) {
				...
				if (pointsAry[0].x - this.offset.x > 0 && pointsAry[0].x - this.offset.x < this.canvasWidth &&
					...
					while (pointsAry[0].x - this.offset.x > 0 && pointsAry[0].x - this.offset.x < this.canvasWidth &&
						...
						yy = this.unitsGapY * this.functions[i].compiled.evaluate({ x: xx / this.unitsGapX });
		...
		this.updateHover = function (e) {
			e.preventDefault();
			e.stopPropagation();
			var mouseX = parseInt(e.clientX - this.canvas.offsetLeft), mouseY = parseInt(e.clientY - this.canvas.offsetTop);
		...
		var doingResize = false, isInterval = 0, parent = this;
		parent.mouseState = {};
