...
k,b+d);this.quadraticCurveTo(a,b+d,a,b+d-k);this.lineTo(a,b+k);this.quadraticCurveTo(a,b,a+k,b);this.closePath();g&&this.stroke();e&&this.fill()};this.canvas=u.canvas;this.ctx=u;this.initialized=!1;this.functions=[];this.offset={x:0,y:0};this.zoom=0;this.options=h.merge(n,v);this.functionsComp=h.complileTemplate(this.options.toopTips.template);this.canvasWidth=this.canvas.width=this.options.size.width();this.canvasHeight=this.canvas.height=this.options.size.height();n={};this.drawGrid=function(){this.ctx.clearRect(0,
0,this.canvas.width,this.canvas.height);var a=this.canvasWidth=this.canvas.width,b=this.canvasHeight=this.canvas.height,c=this.options.gridSettings.range.xM+this.zoom,d=this.options.gridSettings.range.yM+this.zoom,k=-1*c+(this.options.gridSettings.range.xP-this.zoom)+1,e=-1*d+(this.options.gridSettings.range.yP-this.zoom)+1,g=a/k,f=b/e,l=g*(-1*c+1)-g/2,h=b-(f*(-1*d+1)-f/2),t=this.offset.x/g,p=-1*this.offset.y/f,y=this.offset.x>a/2?0:-1*this.offset.x>a/2?a:l-this.offset.x,n=this.offset.y>b/2?0:-1*
this.offset.y>b/2?b:h-this.offset.y,m=-1*this.offset.x+20>a/2?-10:10,r=this.offset.y+20<b/2?-10:20;this.initialized&&this.ctx.restore();this.ctx.save();this.ctx.lineWidth=2;this.ctx.strokeStyle="#7f8c8d";this.offset.y<b/2&&-1*this.offset.y<b/2&&(this.ctx.beginPath(),this.ctx.moveTo(0,h-this.offset.y),this.ctx.lineTo(a,h-this.offset.y),this.ctx.stroke());this.offset.x<a/2&&-1*this.offset.x<a/2&&(this.ctx.beginPath(),this.ctx.moveTo(l-this.offset.x,0),this.ctx.lineTo(l-this.offset.x,b),this.ctx.stroke());
if(this.options.gridSettings.referenceLines){for(a=Math.floor(t);a<k+Math.floor(t+.5);a++){var q=g*a+g/2-this.offset.x;0!=a+c&&(this.ctx.lineWidth=2,this.ctx.strokeStyle=this.options.gridSettings.colors.axes,this.ctx.beginPath(),this.ctx.moveTo(q,0),this.ctx.lineTo(q,this.canvasHeight),this.ctx.stroke())}for(a=Math.floor(p);a<e+Math.floor(p+.5);a++)q=b-(f*a+f/2)-this.offset.y,0!=a+d&&(this.ctx.lineWidth=2,this.ctx.strokeStyle=this.options.gridSettings.colors.axes,this.ctx.beginPath(),this.ctx.moveTo(0,
q),this.ctx.lineTo(this.canvasWidth,q),this.ctx.stroke())}for(a=Math.floor(t);a<k+Math.floor(t+.5);a++)0!=a+c&&this.drawUnitX(g*a+g/2-this.offset.x,n,a+c,r);for(a=Math.floor(p);a<e+Math.floor(p+.5);a++)0!=a+d&&this.drawUnitY(y,b-(f*a+f/2)-this.offset.y,a+d,m);this.initialized=!0;this.middleX=l;this.middleY=h;this.unitsGapX=g;this.unitsGapY=f};this.drawUnitX=function(a,b,c,d){this.ctx.lineWidth=2;this.ctx.strokeStyle=this.options.gridSettings.colors.temLines;this.ctx.beginPath();this.ctx.moveTo(a,
...
for(a=0;a<this.functionsComp.length;a++)this.ctx.fillText(this.functionsComp[a](b),d+5,e+25+35*a,140);this.hoverMenu=!0;return}this.hoverMenu&&(this.updateGraths(),this.hoverMenu=!1)}}};this.updateOffset=function(a,b){"undefined"!==a&&"undefined"!==b&&(this.offset.x=parseInt(b.mouseX-(a.clientX-this.canvas.offsetLeft)),this.offset.y=parseInt(b.mouseY-(a.clientY-this.canvas.offsetTop)));this.updateGraths();this.addMissingPoints()};this.updateZoom=function(a){a=window.event||a;this.zoom+=Math.max(-1,
Math.min(1,a.wheelDelta||-a.detail));if(-1<this.options.gridSettings.range.xM+this.zoom||1>this.options.gridSettings.range.xP-this.zoom||-1<this.options.gridSettings.range.yM+this.zoom||1>this.options.gridSettings.range.yP-this.zoom)--this.zoom;else{this.drawGrid();for(var b=0;b<this.functions.length;b++)a=this.funGraph(this.functions[b].formula,this.functions[b].color),this.functions[b].points=a.points,this.functions[b].compiled=a.compFunction;this.addMissingPoints()}};this.resizeGraths=function(){this.drawGrid();
...
function(a){e.updateZoom(a)},!1));this.options.changeOnResize&&(window.onresize=function(){if(e.canvasWidth!=e.options.size.width()||e.canvasHeight!=e.options.size.height())r=!0,e.canvasWidth=e.canvas.width=e.options.size.width(),e.canvasHeight=e.canvas.height=e.options.size.height(),e.resizeGraths(),r=!1});return n};
